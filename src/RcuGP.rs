...
pub struct RcuGPShared <T: ?Sized>
{
...
pub struct RcuGpReadGuard<'a, T: ?Sized + 'a> {
...
impl <'a, T: ?Sized + 'a> RcuGpReadGuard<'a, T> 
{
    pub fn new(lock:&'a RcuGPLock<T>) -> Self {
    
        return RcuGpReadGuard{
            data:unsafe { NonNull::new_unchecked(lock.global_info.data.get()) },
            inner_lock:lock};
    }
}

impl<T: ?Sized> Deref for RcuGpReadGuard<'_, T> {
    type Target = T;
...
impl<'a,T:?Sized> Drop for RcuGpReadGuard<'a,T>
{
fn drop(&mut self)
{
...
impl <T: ?Sized> RcuGPLock<T> {
    ...
    fn read_lock(&self) 
    {
        ...
        {
            self.global_info.thread_ctr[id].store(self.global_info.global_ctr.load(Ordering::Acquire) + RCU_NEST_COUNT,Ordering::SeqCst );
            smp_mb();
        }
        else 
        {
    ...
    {   
        ...
        self.global_info.global_ctr.store(new_value ^ RCU_GP_CTR_PHASE,Ordering::Release );
        barrier();
